<!-- Footer -->
<div class="container-fluid bg-white mt-5">
    <div class="row">
        <div class="col-lg-4 p-4">
            <h3 class="fw-bold fs-3 mb-2 text-danger">BSU LAMS</h3>
            <p>
                Laboratory Asset Management System for the Hot Kitchen, Cold Kitchen, Food & Beverage Services, and Laundry facilities of Batangas State University.
            </p>
        </div>
        <div class="col-lg-4 p-4">
            <h5 class="mb-3 text-dark">Links</h5>
            <a href="<?= BASE_URL ?>/index.php" class="d-inline-block mb-2 text-dark text-decoration-none">Home</a><br>
            <a href="<?= BASE_URL ?>/reserve.php" class="d-inline-block mb-2 text-dark text-decoration-none">Make Reservation</a><br>
            <a href="<?= BASE_URL ?>/my_reservations.php" class="d-inline-block mb-2 text-dark text-decoration-none">My Reservations</a><br>
        </div>
        <div class="col-lg-4 p-4">
            <h5 class="mb-3 text-dark">Contact Us</h5>
            <p>BSU Main Campus, Nasugbu Batangas</p>
            <p>Email: laboratory@bsu.edu.ph</p>
            <p>Phone: +63 123 456 7890</p>
        </div>
    </div>
</div>

<h6 class="text-center bg-danger text-white m-0 p-3">Designed and Developed for BSU - All Rights Reserved</h6>

<script src="<?= BASE_URL ?>/assets/js/main.js"></script>
</body>
</html>
